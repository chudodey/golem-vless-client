...
try {
  ...
  & python "$root\bin\render_config.py" --endpoints "$root\config\endpoints.txt" --policy "$root\config\policy.conf" --out "$root\state\config.json" --state-dir "$root\state" --fetch --tun-stack mixed --log-level warn 1>> $log 2>> $log
  if ($LASTEXITCODE -ne 0) { throw "config render failed: $LASTEXITCODE" }
  & "$root\bin\sing-box.exe" check -c "$root\state\config.json" 1>> $log 2>> $log
  if ($LASTEXITCODE -ne 0) { throw "sing-box check failed: $LASTEXITCODE" }
  & "$root\bin\sing-box.exe" run -c "$root\state\config.json" 1>> $log 2>> $log
