...
- **Проверка живости нод** — при сборке каждая нода реально проверяется
  запросами к `api.anthropic.com` (ждать 401/404), `www.youtube.com` и
  `openrouter.ai/api/v1/models` (ждать 200); в гонку `urltest` попадают
  только прошедшие проверку (B-010).
...
- **Автовосстановление** — на Linux systemd перезапускает клиент при падении;
  на Windows дежурный watchdog каждые 5 минут проверяет канал и после трёх
  последовательных сбоев перезапускает клиент. Каждая проверка и причина
  восстановления сохраняются в журнале; `status` объясняет текущий пул нод.
...
Перед каждым запуском, перезапуском, watchdog-восстановлением и авто-refresh
Golem проверяет внешние VPN/TUN (Durev, Happ и распространённые клиенты). При
обнаружении он **ничего не останавливает и не меняет**, а пишет причину в
`%LOCALAPPDATA%\GolemVLESS\state\connection-events.jsonl` и завершает свою
операцию. Это защищает рабочее соединение, через которое ведётся отладка.
...
| `GolemVpn.ps1 watchdog` | история проверок, авто-восстановлений и смен ноды |
...
### Терминальный мастер «страна выхода» на Windows (B-019)

Тот же режим, но без ручного редактирования файлов — отдельное терминальное
окно с мастером (один UAC при запуске):

1. **Установить** (копирует скрипт + обновлённый `render_config.py` в
   `%LOCALAPPDATA%\GolemVLESS\bin` и кладёт ярлык на рабочий стол):

   ```powershell
   .\windows\install-country-switch.ps1
   ```

   (на свежих установках ярлык «VPN страна выхода (оплата)» появляется в папке
   `Golem VPN Windows` уже из `Install-GolemVless.ps1`).
2. **Запустить ярлык** → мастер показывает текущий фильтр и ведёт по шагам:
   выбор страны (US/GB/US+GB/своя) → список нод под фильтром со временем
   отклика каждой → подтверждение → `countries = …` в `policy.conf` и
   перезапуск клиента (прогресс в этом же окне).
3. Окно «замирает»: можно заниматься делом (клиент уже в нужной стране).
4. По **[Enter]** мастер возвращает прежний режим (`countries = `), перезапускает
   клиент и закрывается. Даже если закрыть окно через `q` или оно завершится
   неожиданно, watchdog автоматически снимет временный фильтр через 30 минут
   (время можно поменять параметром `-DurationMinutes`, от 5 до 240).

Параметр командной строки для быстрого запуска: `CountrySwitch.ps1 US`
(пропускает вопрос о стране). Ноды со временем отклика берутся из временного
рендера с `--probe-out` — работающий клиент не затрагивается.
...
### Что делает watchdog на Windows

Задача `GolemVLESS-Watchdog` запускается раз в 5 минут. Она проверяет, что
`sing-box` (и `xray`, если он нужен текущему пулу) запущены, а запрос через
локальный VPN-прокси получает HTTP 204. Разовая ошибка не вызывает перезапуск:
счётчик должен дойти до 3 подряд, чтобы избежать ненужных разрывов из-за
краткого сбоя сети.

```powershell
& "$env:LOCALAPPDATA\GolemVLESS\bin\GolemVpn.ps1" watchdog
& "$env:LOCALAPPDATA\GolemVLESS\bin\GolemVpn.ps1" status
```

`status` показывает последний запуск watchdog, серию сбоев, число проверок,
перезапусков и смен ноды в сохранённом журнале, а также почему сформирован
именно текущий пул нод. Файл истории: `%LOCALAPPDATA%\GolemVLESS\state\watchdog-events.jsonl`.
У `urltest` конкретный победитель выбирается внутри sing-box и может меняться
без перезапуска; поэтому статус честно показывает режим и число кандидатов, а
не выдаёт стартовую ноду рендера за фактический live-выход.

Для безопасной отладки используется такой цикл: включить Durev/Happ и
передать результат диагностических команд; затем выключить внешний VPN,
дождаться, что Golem preflight разрешит запуск, запустить Golem и проверить
соединение. Если соединения нет — остановить только Golem, вернуть внешний
VPN и сообщить результат. Golem не пытается сам переключать эти два режима.

Ярлыки управления держат окно открытым после команды, включая ошибку. `stop`
явно проверяет, что локальный порт Golem `127.0.0.1:2080` закрыт; он не ищет
и не завершает процессы Durev/Happ по общему имени движка.
