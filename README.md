...
### Временная привязка к стране выхода

Например, для оплаты картой: сервис (ChatGPT и пр.) сверяет страну карты со
страной IP, поэтому на время оплаты удобно ограничить выезд одной-двумя
странами. В `[auto-select]`:

```ini
countries = US            # только ноды США
countries = US, GB        # США и Великобритания
countries =               # пусто — фильтр выключен (вариант по умолчанию)
```

Автовыбор (`urltest`) при этом **остаётся включённым** — он просто гоняет
только ноды из списка; если живая US-нода одна, urltest деградирует в
закреплённую ноду, а не прыгает на «любую быструю». Фильтр применяется до
всех проб и до сборки обоих конфигов, поэтому TCP/HTTP-пробы, пул `urltest`
и пул xhttp-нод (через xray) видят только разрешённые страны. Страна
определяется по имени ноды (флаг-эмодзи, «United States», «USA», «US 03»,
«США» и т.п.); ноды с нераспознанной страной под активным фильтром
**исключаются** (fail-closed — во время оплаты лишние сюрпризы не нужны).

Порядок действий:

1. **Проверить заранее**, какие ноды попадут под фильтр (не трогая работающий
   клиент):
   ...
   (на Windows: `python scripts\render_config.py --endpoints "$env:LOCALAPPDATA\GolemVLESS\config\endpoints.txt" --check-only`)
2. Вписать `countries = US` (или `US, GB`) → применить.
3. **После оплаты** — убрать значение (`countries =`) → применить.

Применение: Linux — `sudo vpnctl reload`; Windows — `GolemVpn.ps1 restart`.

⚠️ Учтите, что у провайдера может быть мало US-нод: у текущей подписки Durev
свободных выходов в США практически один-два (основной «USA» + «USA 66» —
маршрут «Белые списки», это YouTube без рекламы, не универсальный выход).
Поэтому `countries = US` оставляет в гонке 1–2 ноды; добавление `GB` даёт
запасной вариант. И страна IP — лишь один из сигналов антифрода Stripe:
адрес биллинга карты и отпечаток браузера тоже участвуют, так что совпадение
«US-карта + US-IP» правильно, но саму решимость платежа гарантирует только
сам эквайер.
