...
**О нодах подписки Durev:** основные ноды используют транспорт xhttp, который
sing-box не поддерживает (см. Troubleshooting). Рабочие — ws-ноды
`cdn.assetrelay.store:443` (метка «Белые списки»): они открывают
Anthropic/OpenAI (`401`) и YouTube (`200`), но не все — часть заблокирована
(403) или не держит YouTube. В `endpoints.txt` уже отфильтрованы только
проверенные 401/200-ноды, а `urltest` выбирает из них живую. **Список нод
меняется**: нода, которая сегодня работает, завтра может давать 403 или
пропасть — поэтому время от времени пересобирайте конфиг с `--fetch` и
повторяйте ручную проверку (см. B-010 в `docs/backlog.yaml`; автопроверка
каждой ноды при сборке — в планах).

1. В этой рабочей копии положите свою подписку в
   `secrets/endpoints.txt` (файл игнорируется Git). Если переносите ключ с
   Dell, скопируйте этот локальный файл; не добавляйте его в коммит.
...
4. Проверка: откройте Claude/Codex и выполните в обычном PowerShell
...
реализует** xhttp/splithttp — такие ссылки рендерер отбрасывает с
предупреждением `WARN: skip ... xhttp transport is not supported`. Для таких
подписок берите ws/tcp-ноды (у Durev это CDN-ноды `cdn.assetrelay.store`,
которые открывают Anthropic/OpenAI: `401`, YouTube: `200`).
