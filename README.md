...
- **Телеметрия провайдера 24/7** (B-015) — `stats.py` каждые 30 минут
  записывает живость, пинг и HTTP-доступность каждой ноды; сводка по
  провайдерам, странам и времени жизни нод накапливается месяцами.
...
| `vpnctl stats` | собрать телеметрию нод и показать сводку (B-015) |
| `vpnctl report` | показать сводку телеметрии без нового сбора |
...
`install.sh` ставит systemd-таймер `golem-vless-stats.timer`, который каждые
30 минут собирает данные (`golem-vless-stats.service`, oneshot): TCP-пинг и
HTTP-проверку каждой ноды, фиксирует появления/исчезновения нод и текущий
выбор клиента. Журнал append-only событий лежит в
...
| Команда | Что делает |
|---|---|
| `vpnctl stats` | собрать свежие данные и показать сводку |
| `vpnctl report` | показать сводку (даннные уже собраны) |
| `python3 scripts/stats.py report --state-dir /var/lib/golem-vless --months 1` | сводка за месяц |
...
месяцам — то есть какой провайдер реально стабильнее, а не кажется. На
Windows запустить руками: `GolemVpn.ps1 stats` или `GolemVpn.ps1 report`
(плюс ярлыки «VPN stats»/«VPN report»).
